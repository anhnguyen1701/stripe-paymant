...
export function Payments() {
  ...
  const handleSubmit = async (event) => {
    ...
    const { paymentIntent: updatedPaymentIntent, error } = await stripe.confirmCardPayment(paymentIntent.client_secret, {
  ...
  return (
    <>
      ...
      <div className="well">
        ...
        <p>Change the amount of the payment in the form, then request a Payment Intent to create context for one-time payment. Min 50, Max 9999999</p>

        <div className="form-inline">
          <input className="form-control" type="number" value={amount} disabled={paymentIntent} onChange={(e) => setAmount(e.target.value)} />
          <button className="btn btn-success" disabled={amount <= 0} onClick={createPaymentIntent} hidden={paymentIntent}>
...
function PaymentIntentData(props) {
  if (props.data) {
    ...
    return (
      <>
        <h3>
          Payment Intent <span className={'badge ' + (status === 'succeeded' ? 'badge-success' : 'badge-secondary')}>{status}</span>
        </h3>
        <pre>
          ID: {id} <br />
